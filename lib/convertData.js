...
export const timeAgo = (createdTime) => {
    const currentTime = new Date();
    const timeDifference = Math.floor((currentTime - new Date(createdTime)) / 1000);
    ...
    for (const format of timeFormats) {
        const interval = Math.floor(timeDifference / format.seconds);
        if (interval >= 1) {
